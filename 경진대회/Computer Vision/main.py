...
class Custom_dataset(Dataset):  #왜 커스텀시킴?
...
train_png = sorted(glob("./open/train/*.png"))  # 이미지 파일 이름 전부 다 가져오기 <clss: list>
test_png = sorted(glob("./open/test/*.png"))  # 이미지 파일 이름 전부 다 가져오기 <clss: list>
...
label_unique = {key: value for key, value in zip(label_unique, range(len(label_unique)))}  # 라벨을 key:value 매칭 숫자로(0~88까지)
